...
2. **Spawn 1 debugger** (`Task`, `general-purpose`). Prompt:
   > Answer TERSE. Read `.opencode/agents/debugger.md` and follow it exactly.
...
back to `/build`. The test-gate plugin (`.opencode/plugins/test-gate.ts`) blocks
`session.idle` until tests actually run.
